...
OrientationInfo get_orientation(AVStream *stream)
{
    ...
    for (int i = 0; i < stream->codecpar->nb_coded_side_data; i++) {
        ...
        if (sd->type == AV_PKT_DATA_DISPLAYMATRIX && sd->size >= 9*(sizeof(int32_t))) {
            ...
            */
            // 16.16 fixed point
            int64_t a = matrix[0]; int64_t b = matrix[1]; // x and y doesn't matter
            int64_t c = matrix[3]; int64_t d = matrix[4];
            // 2.30 fixed point, for perspective
            // Assume these are 0, 0, 1. They are ignored in most decoders,
            // But apple sometimes still sets these to non-standard values???
            // int32_t u = matrix[6]; int32_t v = matrix[7]; int32_t w = matrix[8];
            // if ((u != 0) || (v != 0) || (w != (1 << 30))) {
            //     theta = 0;
            //     hflip = false;
            // }
            ...
            int64_t det = a * d - b * c;
            // same value for both h/vflip since we only use hflip anyway
            hflip = det < 0;
...
// ================= Packet Reader (non-ASYNCIFY, pointer-based) =================
// Refactored to return a AVPacketReader* directly instead of an integer handle.
// Lifecycle:
// 1) AVPacketReader* read_av_packet_init(...) -> nullptr on error
// 2) read_av_packet_next(reader) -> WebAVPacket | null
// 3) read_av_packet_close(reader) -> releases resources and deletes reader (idempotent)
// The caller MUST NOT use the reader after close.
